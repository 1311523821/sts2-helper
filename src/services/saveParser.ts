...
export class SaveParser {
  ...
  static parseJSON(json: string): ParsedSave {
    ...
    try {
      ...
      const floor = SaveParser.extractNumber(save, 'floor', 1, errors)
      const act = SaveParser.extractNumber(save, 'act', Math.ceil(floor / 16), errors)
      const health = SaveParser.extractNumber(save, 'health', 0, errors)
      const maxHealth = SaveParser.extractNumber(save, 'maxHealth', save.max_health ?? health, errors)
      const gold = SaveParser.extractNumber(save, 'gold', save.coins ?? 0, errors)
      const deck = SaveParser.extractDeck(save, errors)
      const relics = SaveParser.extractRelics(save, errors)
  ...
  static parseText(text: string): ParsedSave {
    ...
    const deck = SaveParser.parseCardList(deckStr, errors)
  ...
  private static extractNumber(data: Record<string, unknown>, key: string, fallback: number, errors: string[]): number {
  ...
  private static extractRelics(data: Record<string, unknown>, errors: string[]): OwnedRelic[] {
  ...
  private static parseCardList(str: string, errors: string[]): DeckCard[] {
    if (!str) return []

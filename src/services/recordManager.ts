...
export class RecordManager {
  ...
  /**
   * 获取最近记录（分页查询，按 startTime 降序）
   * 使用 IndexedDB cursor 实现高效分页
   */
  async getRecentRecords(limit: number = 20, offset: number = 0): Promise<GameRecord[]> {
    return new Promise((resolve, reject) => {
      const tx = this.getDB().transaction(STORE_NAME, 'readonly')
      const store = tx.objectStore(STORE_NAME)
      const index = store.index('startTime')
      // 按 startTime 降序遍历（最新的在前）
      ...
      cursorRequest.onsuccess = () => {
        ...
        if (results.length < limit) {
          results.push(cursor.value as GameRecord)
          cursor.continue()
        } else {
          resolve(results)
        }
      }
      cursorRequest.onerror = () => reject(new Error('读取记录失败'))
    })
  }

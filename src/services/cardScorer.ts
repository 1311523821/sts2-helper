import type { CardOption, DeckCard, ArchetypeMatch, CharacterId, OwnedRelic, ScoreDimensions, CardScore, SkipAnalysis, Recommendation } from '@/types'
...
function calcDeckHealthContribution(card: { type: string; cost: number }, deck: DeckCard[]): number {
  const ratio = calcCurrentRatio(deck)
  let score = 50

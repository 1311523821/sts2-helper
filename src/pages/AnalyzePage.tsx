...
export default function AnalyzePage() {
  ...
  // Deck stats — hooks must be called before any early return
  const stats = useMemo(() => {
  ...
  // Group deck cards
  const allCardsForGroup = character ? getCardsByCharacter(character) : []
  const deckCardsForGroup = deck.map(dc => ({ ...dc, card: allCardsForGroup.find(c => c.id === dc.cardId) }))
  const groupedDeck = useMemo(() => {
    const groups: Record<string, typeof deckCardsForGroup> = {}
    for (const dc of deckCardsForGroup) {
      if (!dc.card) continue
      let key: string

...
function loadHistory(): SimHistory {
  try {
    ...
    return { total: 0, correct: 0, streak: 0, bestStreak: 0, results: [] };
  }
}
...
export default function SimulatorPage() {
  ...
  const handleSelect = useCallback(
    (cardId: string) => {
      ...
      if (!scenario) return;
      const scores = scenario.options.map((c) => ({
        id: c.id,
        score: simpleScore(c, scenario.floor),
      }));
    ...
    return (
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <p style={{ color: 'var(--text-secondary)' }}>模拟选牌场景，训练你的选牌直觉</p>
        </div>
        ...
        {history.total > 0 && (
          ...
          >
            ...
            <div className="grid grid-cols-4 gap-3">
              ...
              <div className="text-center">
                <div
                  className={`text-2xl font-bold ${isStsTheme ? 'text-[#c9a227]' : 'text-xm-primary'}`}
                >
                  {history.total > 0 ? Math.round((history.correct / history.total) * 100) : 0}%
                </div>
                <div className="text-xs" style={{ color: 'var(--text-muted)' }}>
                  正确率
                </div>
              ...
              <div className="text-center">
                <div
                  className={`text-2xl font-bold ${isStsTheme ? 'text-[#2e8b57]' : 'text-xm-secondary'}`}
                >
                  {history.streak}
                </div>
                <div className="text-xs" style={{ color: 'var(--text-muted)' }}>
              ...
              <div className="text-center">
                <div
                  className={`text-2xl font-bold ${isStsTheme ? 'text-[#9932cc]' : 'text-purple-500'}`}
                >
                  {history.bestStreak}
                </div>
                <div className="text-xs" style={{ color: 'var(--text-muted)' }}>
        ...
        {showHistory && history.results.length > 0 && (
          <div
            className={`mt-6 ${isStsTheme ? 'sts-filter-panel xiaomi-card-static' : 'xiaomi-card-static'}`}
          >
            <h3 className="font-bold mb-4" style={{ color: 'var(--text-primary)' }}>
              最近记录
            </h3>
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm" style={{ color: 'var(--text-primary)' }}>
                        选了 <strong>{chosenCard?.name || r.chosen}</strong>
                      </span>
                    </div>
                    {!r.isCorrect && (
                      <span className="text-xs" style={{ color: 'var(--text-muted)' }}>
                        推荐: {recCard?.name || r.recommended}
                      </span>
                    )}
                  </div>
                );
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1
            className={`text-2xl font-bold ${isStsTheme ? 'sts-title-font' : ''}`}
            style={{ color: 'var(--text-primary)' }}
          >
            🎮 选牌模拟器
          </h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm" style={{ color: 'var(--text-muted)' }}>
            正确率: {history.total > 0 ? Math.round((history.correct / history.total) * 100) : 0}%
          </span>
        </div>
      </div>
      ...
      {/* Scenario display */}
      <div
        className={`mb-6 text-center ${isStsTheme ? 'sts-filter-panel xiaomi-card-static' : 'xiaomi-card-static'}`}
      >
              ...
              {showResult && (
                <div
                  className={`absolute inset-0 rounded-xl border-2 transition-all duration-500 ${
                    isStsTheme
                      ? isBest
                        ? 'border-[#2e8b57] bg-[rgba(46,139,87,0.1)]'
                        : isSelected
                          ? 'border-[#d94437] bg-[rgba(217,68,55,0.1)]'
                          : 'border-transparent'
                      : isBest
                ...
                >
                  <div className="absolute top-2 right-2">
                    <span
                      className={`text-xs font-bold px-2 py-1 rounded-lg ${
                        isStsTheme
                          ? isBest
                            ? 'bg-[rgba(46,139,87,0.2)] text-[#2e8b57]'
                            : isSelected
                              ? 'bg-[rgba(217,68,55,0.2)] text-[#d94437]'
                              : 'bg-[#1a1a2e] text-[#a8a0b4]'
                          : isBest
                            ? 'bg-green-100 text-green-700'
                            : isSelected
                              ? 'bg-red-100 text-red-700'
                              : 'bg-warm-100 text-text-muted'
                      }`}
                    >
                  ...
                  {isBest && (
                    <div
                      className={`absolute bottom-2 left-2 right-2 text-xs rounded-lg p-2 ${
                        isStsTheme
                          ? 'text-[#2e8b57] bg-[rgba(46,139,87,0.15)]'

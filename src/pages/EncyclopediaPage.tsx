...
export default function EncyclopediaPage() {
  ...
  if (costRange !== 'all')
    cards = cards.filter((c) => c.cost >= costRangeObj.min && c.cost <= costRangeObj.max);
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        <h1
          className={`text-3xl font-bold mb-2 ${isStsTheme ? 'sts-title-font' : ''}`}
          style={{ color: 'var(--text-primary)' }}
        >
          📖 卡牌图鉴
        </h1>
        <p style={{ color: 'var(--text-secondary)' }}>
          浏览所有角色的卡牌，按类型、稀有度和费用筛选
        </p>
      </div>
      ...
      {/* 搜索和筛选 */}
      <div
        className={`mb-6 ${isStsTheme ? 'sts-filter-panel xiaomi-card-static' : 'xiaomi-card-static'}`}
      >
        <div className="flex flex-col gap-3">
          ...
          <div className="relative">
            ...
            {search && (
              <button
                onClick={() => setSearch('')}
                className="absolute right-3 top-1/2 -translate-y-1/2 hover:opacity-80 text-sm"
                style={{ color: 'var(--text-muted)' }}
              >
                ✕
              </button>
      ...
      <div className="flex items-center gap-2 mb-4">
        <span className="text-sm" style={{ color: 'var(--text-muted)' }}>
          {charIcon} {info.name} · 共{' '}
          <strong style={{ color: 'var(--text-secondary)' }}>{cards.length}</strong> 张卡牌
          {search && (
            <span className={`ml-2 ${isStsTheme ? 'text-[#c9a227]' : 'text-xm-primary'}`}>
              搜索"{search}"
            </span>
          )}
        </span>
      ...
      {/* 卡牌网格 */}
      <div
        className={`grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 ${isStsTheme ? 'sts-card-grid' : ''}`}
      >
        {cards.map((card) => (
          <div key={card.id} className="relative group">
            ...
            {showCompare && (
              <div
                ...
                onClick={(e: React.MouseEvent) => toggleCompare(card, e)}
              />
            )}
          </div>
        ))}
      ...
      {cards.length === 0 && (
        <div className="text-center py-16" style={{ color: 'var(--text-muted)' }}>
          ...
          <button
            ...
            className={`mt-3 text-sm hover:underline ${isStsTheme ? 'text-[#c9a227]' : 'text-xm-primary'}`}
          >
            清除所有筛选
          </button>
        </div>
      ...
      {showCompare && compareCards.length >= 2 && (
        <ComparePanel
          cards={compareCards}
          onRemove={(id) => setCompareCards((prev) => prev.filter((c) => c.id !== id))}
          isStsTheme={isStsTheme}
        />
      )}
...
function CardDetailModal({
  ...
  // Focus trap: Tab key cycles focus within modal
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm ${
        isStsTheme ? 'sts-modal-overlay bg-black/70' : 'bg-black/40'
      }`}
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        ref={modalRef}
        className={`rounded-2xl max-w-lg w-full max-h-[85vh] overflow-y-auto shadow-2xl ${
          isStsTheme ? 'sts-modal-content bg-[#1a1a2e] border border-[#2a2a3e]' : 'bg-white'
        }`}
      ...
      >
        {/* Header */}
        <div
          className={`p-6 border-b ${isStsTheme ? 'sts-modal-header border-[#2a2a3e]' : 'border-warm-200'}`}
        >
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`w-8 h-8 rounded-full flex items-center justify-center text-lg transition-all ${
                  isStsTheme
                    ? favorited
                      ? 'bg-[#c9a227] text-[#0d0d1a]'
                      : 'bg-[#141428] text-[#a8a0b4] hover:text-[#c9a227]'
                    : favorited
                      ? 'bg-xm-primary text-white'
                      : 'bg-warm-100 text-text-muted hover:text-xm-primary'
                }`}
              >
                {favorited ? '★' : '☆'}
              </button>
              <button
                ...
                className={`w-8 h-8 rounded-full flex items-center justify-center ${
                  isStsTheme
                    ? 'sts-modal-close bg-[#141428] text-[#a8a0b4] hover:bg-[#1f1f32] hover:text-[#c9a227]'
        ...
        <div className="p-6 space-y-4">
          {/* Meta info */}
          <div className="flex flex-wrap gap-2">
            <span className={`tag-pill ${RARITY_TAG_COLORS[card.rarity]}`}>
              {RARITY_NAMES[card.rarity]}
            </span>
            <span className="tag-pill">{TYPE_NAMES[card.type]}</span>
            <span className={`tag-pill ${isStsTheme ? 'bg-[#1f1f32]' : 'bg-warm-100'}`}>
          ...
          {/* Description */}
          <div
            className={isStsTheme ? 'sts-filter-panel xiaomi-card-static' : 'xiaomi-card-static'}
          >
          ...
          {card.upgradedDescription && (
            <div
              className={`xiaomi-card-static ${isStsTheme ? 'border-[#c9a227]/30 bg-[rgba(201,162,39,0.1)]' : 'border-xm-primary/20 bg-xm-light/30'}`}
            >
              <h4
                className={`text-sm font-bold mb-2 ${isStsTheme ? 'text-[#c9a227]' : 'text-xm-primary'}`}
              >
                ⬆ 升级后
              </h4>
              <p className="text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
                    ...
                    <span
                      ...
                      className={`inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-lg font-medium ${
                        isStsTheme
                          ...
                          : 'bg-xm-light text-xm-primary'
                      }`}
                    >
                      {kwData?.icon} {kwData?.name || kw}
                    </span>
                  );
                })}
              </div>
...
function ComparePanel({
  ...
  return (
    <div
      className={`fixed bottom-0 left-0 right-0 z-40 shadow-2xl p-4 ${
        isStsTheme ? 'bg-[#0d0d1a] border-t border-[#2a2a3e]' : 'bg-white border-t border-warm-200'
      }`}
    >
      <div className="max-w-6xl mx-auto">
        <h3
          className={`text-sm font-bold mb-3 ${isStsTheme ? 'sts-title-font-small' : ''}`}
          style={{ color: 'var(--text-primary)' }}
        >
          ⚖ 卡牌对比
        </h3>
          ...
          {cards.map((card) => (
            <div
              ...
              className={`p-3 rounded-xl border ${
                isStsTheme ? 'bg-[#141428] border-[#2a2a3e]' : 'bg-warm-50 border-warm-200'
              }`}
            >
              ...
              <div className="space-y-1 text-xs" style={{ color: 'var(--text-secondary)' }}>
                <div className="flex justify-between">
                  <span>类型</span>
                  <span>
                    {TYPE_ICONS[card.type]} {TYPE_NAMES[card.type]}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>费用</span>
                  <span>{card.cost === -1 ? 'X' : card.cost}</span>
                </div>
                <div className="flex justify-between">
                  <span>稀有度</span>
                ...
                <div className="flex justify-between">
                  <span>关键词</span>
                  <span>{card.keywords.length}</span>
                </div>
                <div className="flex justify-between">
                  <span>标签</span>
                ...
                <p
                  className={`mt-2 pt-2 line-clamp-3 ${
                    isStsTheme ? 'border-t border-[#2a2a3e]' : 'border-t border-warm-200'
                  }`}
                >
                  {card.description}
                </p>
